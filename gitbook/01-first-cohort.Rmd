...
In this chapter, we analyzed complement levels in 24 FSHD subjects and 12 controls from the first cohort.  The tested complement components included six components of the classical/lectin activation pathway (C1q, C4, MBL, C4a, C2, C4b), four of the alternative pathway (Factor H, Factor I, Factor D, Factor B), and four of the terminal effector pathway (C3, C5, C5a, sC5b-9). 
...
```{r demographi-distribution-fshd-1}
...
```{r demographi-distribution-control-1}
...
- visualization of p-values (bar plot; Fig. 2a) and complement levels distribution in each group (violin plot; Fig. 2b)     
- heatmap coupled with hierarchical cluster analysis (Suppl. Fig. S1)     
...
```
```{r cohort1-ttest-barplot, fig.weidth=8, fig.height=8, fig.cap="Cohort1 per-compoment t-tests: FSHD vs. controls. * negative -10Log10(p-value) indicates negative t-statistics."}
ggplot(cohort1_ttest, aes(x=complement, y=log10Pval, fill=candidate)) +
  geom_bar(stat="identity", width=0.7, show.legend=FALSE) +
  coord_flip() +
  scale_fill_manual(values=c("#999999", "#E69F00")) +
  theme_bw() +
  labs(title="Cohort1: t-statistics and p-values", 
...
```{r cohort1-violin, fig.weidth=8, fig.height=8, fig.cap="14 complement components measurements grouped by FSHD and control samples."}
cohort1_tidy %>%
