...
We next analyzed complement component levels in plasma from a second cohort of 41 FSHD subjects and 17 controls. The corresponding dataset is `../data/table_2.rda` and Suppl. Table 2. The 15 tested components in this second assay are Bb, C2, C4b, C5, C5a, Factor D, MBL, Factor I, C1q, C3, C4, Factor B, Factor H, C4a, sC5b-9.
...
- visualization of p-values (bar plot; Fig. 3a) and complement levels distribution in each group (violin plot; Fig. 3b)     
...
ggplot(cohort2_ttest, aes(x=complement, y=log10Pval, fill=candidate)) +
  ...
  theme_bw() +
  labs(title="Cohort2: t-statistics and p-values", 
...
## Complement levels of controls from two cohorts
Suppl. Fig. 2a
...
The first and the second visits are names `plasma1` and `plasma2` respectively. The code chuck below reproduce Suppl. Fig. 2b. 
